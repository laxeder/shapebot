...
export default class WorkoutRepository implements DataRepository<Workout> {
  ...
  public async create(workout: Workout): Promise<void> {
    ...
    if (workoutData.status == DataStatus.Enabled) {
      throw new ClientError(`workout "${workout.id}" already exists`, "Um exercício com esse número já existe em nosso sistema");
    }
    ...
    await this.db.save("/workouts", workout);
  }
  ...
  public async update(workout: Partial<Workout>): Promise<void> {
    ...
    if (workoutData.status != DataStatus.Enabled) {
      throw new ClientError(`Not allowed to update workout "${workout.id}"`, "Não foi possível atualizar os dados do exercício");
    }
    ...
    await this.db.save("/workouts", workoutData);
  }
   ...
   * @throws ClientError se o ID do exercício não estiver definido
   */
  public async read(workout: Workout): Promise<Workout> {
    ...
    const workoutData = await this.db.get("/workouts", workout.id);
  ...
  public async delete(workout: Workout): Promise<void> {
    ...
    if (workoutData.status != DataStatus.Enabled) {
      throw new ClientError(`Workout "${workout.id}" has already been deleted`, "Os dados do exercício já foram deletados");
    }
  ...
  public async restore(workout: Workout): Promise<void> {
    ...
    if (workoutData.status != DataStatus.Disabled) {
      throw new ClientError(`Workout "${workout.id}" has already been restored`, "Os dados do exercício já foram restaurados");
    }
  ...
  public async findAll(): Promise<Workout[]> {
    const list = await this.db.findAll("/workouts");
